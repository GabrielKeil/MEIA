...
- Describe the variables with classical and robust summaries and simple
visuals.

- Run PCA on the raw scale and on standardized variables; keep ≥95\%
total variance.

- Study the effect of an injected outlier using classical PCA and a
robust PCA (MCD).
...
\NormalTok{n\_obs }\OtherTok{\textless{}{-}} \FunctionTok{nrow}\NormalTok{(machines\_sub)}
\NormalTok{n\_vars }\OtherTok{\textless{}{-}} \FunctionTok{ncol}\NormalTok{(machines\_sub) }\SpecialCharTok{{-}} \DecValTok{1}                    \CommentTok{\# numeric variables only}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{head}\NormalTok{(machines\_sub}\SpecialCharTok{$}\NormalTok{machine, }\DecValTok{5}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{md }\OtherTok{\textless{}{-}} \FunctionTok{mahalanobis}\NormalTok{(}
\NormalTok{  num\_vars,}
...
Interpretation: most machines sit well below the 97.5\% chi-square
cutoff (≈17.5 for 8 vars), suggesting a fairly homogeneous core. About 5
machines breach the line (one extreme around index 25, another near 13,
plus a few clustered neighbors), meaning their joint profiles are
atypical -- e.g., unusual combinations of cycle time, memory, and cache.
Because classical mean/covariance can mask extremes, a robust
Mahalanobis distance (MCD) would likely push these points even higher
and could flag borderline cases too. To dig deeper, we can inspect those
machine IDs and build contribution plots to see which variables drive
their distances.
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{machine\_ids }\OtherTok{\textless{}{-}} \FunctionTok{rownames}\NormalTok{(machines)[}\DecValTok{71}\SpecialCharTok{:}\DecValTok{111}\NormalTok{]  }\CommentTok{\# names for this slice of the data}
\NormalTok{flagged }\OtherTok{\textless{}{-}} \FunctionTok{which}\NormalTok{(md }\SpecialCharTok{\textgreater{}}\NormalTok{ cutoff)}

\FunctionTok{data.frame}\NormalTok{(}
  \AttributeTok{index =}\NormalTok{ flagged,}
  \AttributeTok{machine =}\NormalTok{ machine\_ids[flagged],}
  \AttributeTok{distance =} \FunctionTok{round}\NormalTok{(md[flagged], }\DecValTok{2}\NormalTok{)}
\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
  index            machine distance
1    13 honeywell-dps:6/96    22.52
2    25         ibm-3033:u    26.82
3    26           ibm-3081    23.13
4    27         ibm-3081:d    23.20
5    28         ibm-3083:b    19.11
\end{verbatim}

These IDs correspond to the spikes in the plot; each mixes unusually
large/small values across the hardware specs. Next step: contribution
plots (variable-wise breakdown of the distance) or a robust Mahalanobis
distance to confirm and prioritize which are true multivariate outliers.
...
\pandocbounded{\includegraphics[keepaspectratio]{report_files/figure-pdf/unnamed-chunk-16-1.pdf}}
...
Suggestion for the written summary (fill in after rendering): - Data
shape and key variable ranges; any clear skew or heavy tails. - Which
PCA you selected (raw vs standardized), how many PCs kept for 95\%, and
how you interpret PC1/PC2 loadings. - What changed after adding the
outlier; how robust PCA mitigated it. - Brief limitations (small sample,
historical hardware data) and next steps (e.g., more robust checks or
